...
from konte.domain import Chunk, ContextualizedChunk, RetrievalRequest
from konte.retrieval.reranker import RerankOutcome
...
class TestReciprocalRankFusion:
    ...
    def test_single_list(self, sample_chunks):
        ...
        from konte.retrieval.fusion import reciprocal_rank_fusion
...
class TestDetermineSuggestedAction:
    ...
    def test_high_score_delivers(self):
        ...
        from konte.retrieval.response import suggested_action

        assert suggested_action(0.9) == "deliver"
        assert suggested_action(0.7) == "deliver"

    def test_medium_score_query_more(self):
        """Test that medium scores suggest 'query_more'."""
        from konte.retrieval.response import suggested_action

        assert suggested_action(0.6) == "query_more"
        assert suggested_action(0.4) == "query_more"

    def test_low_score_refine_query(self):
        """Test that low scores suggest 'refine_query'."""
        from konte.retrieval.response import suggested_action

        assert suggested_action(0.3) == "refine_query"
        assert suggested_action(0.0) == "refine_query"


def _ranked(results):
    """Wrap results whose ranking scores happen to be absolute, as FAISS's are."""
    from konte.retrieval.fusion import Ranked, by_chunk_id
...
class TestBuildRetrievalResponse:
    ...
    def test_builds_response_with_results(self, sample_chunks):
        ...
        from konte.retrieval.response import build_response
        ...
        response = build_response("test query", _ranked(results), top_k=3)
    ...
    def test_builds_response_empty_results(self):
        ...
        response = build_response("test query", Ranked([], {}), top_k=10)
    ...
    def test_respects_top_k(self, sample_chunks):
        ...
        response = build_response("test", _ranked(results), top_k=2)
    ...
    def test_confidence_ignores_the_ranking_scores(self, sample_chunks):
        ...
        response = build_response("test", Ranked(results, measured), top_k=2)
    ...
    def test_unmeasured_chunk_scores_zero(self, sample_chunks):
        ...
        response = build_response("test", Ranked(results, {}), top_k=1)
...
class TestRetrieverModes:
    ...
    def test_retrieve_semantic_empty_store(self):
        ...
        response = retriever.retrieve(RetrievalRequest("test query", mode="semantic"))
    ...
    def test_retrieve_lexical_empty_store(self):
        ...
        retriever = Retriever(faiss_store=None, bm25_store=None)
        response = retriever.retrieve(RetrievalRequest("test query", mode="lexical"))
    ...
    def test_retrieve_hybrid_no_stores(self):
        ...
        retriever = Retriever(faiss_store=None, bm25_store=None)
        response = retriever.retrieve(RetrievalRequest("test query", mode="hybrid"))
    ...
    def test_retrieve_default_mode_is_hybrid(self):
        ...
        retriever = Retriever()
        response = retriever.retrieve(RetrievalRequest("test query"))

        # Should complete without error, even with empty stores
        assert response.query == "test query"

    def test_reranking_has_no_blocking_form(self):
        """Test the sync entry point refuses what it cannot do off the loop."""
        from konte.retrieval.retriever import Retriever

        with pytest.raises(ValueError, match="aretrieve"):
            Retriever().retrieve(RetrievalRequest("test query", rerank=True))
...
class TestConfidenceIsNotTheRanking:
    ...
    def test_weak_hybrid_evidence_does_not_deliver(self):
        ...
        response = retriever.retrieve(
            RetrievalRequest("gibberish xyzzy", top_k=3, use_keyword_extraction=False)
        )
    ...
    def test_hybrid_agrees_with_semantic_on_the_same_evidence(self):
        ...
        semantic = retriever.retrieve(RetrievalRequest("gibberish xyzzy", mode="semantic", top_k=3))
    ...
    def test_lexical_only_evidence_is_measured_by_coverage(self):
        ...
        response = retriever.retrieve(
            RetrievalRequest("quarterly revenue", top_k=3, use_keyword_extraction=False)
        )
    ...
    def test_lexical_mode_separates_a_match_from_a_miss(self):
        ...
        matched = retriever.retrieve(
            RetrievalRequest(
                "quarterly revenue", mode="lexical", top_k=3, use_keyword_extraction=False
            )
        )
        absent = retriever.retrieve(
            RetrievalRequest(
                "quarterly zirconium tungsten",
                mode="lexical",
                top_k=3,
    ...
    async def test_unreachable_reranker_does_not_read_as_relevance(self):
        ...
        with patch(
            "konte.retrieval.retriever.rerank_chunks_with_score", new_callable=AsyncMock
        ) as rerank:
            ...
            response = await retriever.aretrieve(
                RetrievalRequest("query", mode="semantic", top_k=2, rerank=True)
            )
...
class TestFusionWeights:
    ...
    def _top_chunk_id(self, retriever):
        response = retriever.retrieve(
            RetrievalRequest("cash flow", top_k=1, use_keyword_extraction=False)
        )
...
def extractor():
    ...
    with (
        patch("konte.retrieval.query_processor.extract_search_keywords") as sync_extract,
        patch(
            "konte.retrieval.query_processor.extract_search_keywords_async",
...
def _keyword_extraction(enabled: bool):
    ...
    return patch("konte.retrieval.query_processor.settings.BM25_KEYWORD_EXTRACTION", enabled)
...
class TestKeywordExtractionControl:
    ...
    def test_caller_can_turn_extraction_off(self, lexical_retriever, extractor):
        ...
        lexical_retriever.retrieve(RetrievalRequest("test query", use_keyword_extraction=False))

        extractor.assert_not_called()

    def test_caller_can_turn_extraction_on(self, lexical_retriever, extractor):
        """Test an explicit True is honoured."""
        lexical_retriever.retrieve(RetrievalRequest("test query", use_keyword_extraction=True))
    ...
    def test_unset_follows_configuration(self, lexical_retriever, extractor):
        ...
        with _keyword_extraction(False):
            lexical_retriever.retrieve(RetrievalRequest("test query"))
    ...
    def test_semantic_mode_never_extracts(self, lexical_retriever, extractor):
        ...
        lexical_retriever.retrieve(
            RetrievalRequest("test query", mode="semantic", use_keyword_extraction=True)
        )
    ...
    def test_no_lexical_index_never_extracts(self, extractor):
        ...
        Retriever(faiss_store=None, bm25_store=None).retrieve(
            RetrievalRequest("test query", use_keyword_extraction=True)
        )
    ...
    def test_empty_extraction_keeps_the_raw_query(self, lexical_retriever, extractor):
        ...
        with patch.object(
            ...
            lexical_retriever.retrieve(
                RetrievalRequest("what is it", mode="lexical", use_keyword_extraction=True)
            )
...
class TestAsyncRetrieval:
    """Test that the async entry point stays off the blocking extractor."""

    async def test_aretrieve_awaits_extraction(self, lexical_retriever, extractor):
        """Test async retrieval never calls the sync, loop-blocking variant."""
        await lexical_retriever.aretrieve(
            RetrievalRequest("test query", use_keyword_extraction=True)
        )
    ...
    async def test_aretrieve_matches_retrieve(self, lexical_retriever, extractor):
        """Test both entry points rank identically for the same request."""
        request = RetrievalRequest("test query", use_keyword_extraction=False)
        sync = lexical_retriever.retrieve(request)
        result = await lexical_retriever.aretrieve(request)
    ...
    async def test_rerank_hybrid_keeps_the_raw_query(self, lexical_retriever, extractor):
        ...
        with patch(
            ...
            await lexical_retriever.aretrieve(
                RetrievalRequest("test query", mode="hybrid", rerank=True)
            )
    ...
    async def test_rerank_hybrid_honours_an_explicit_flag(self, lexical_retriever, extractor):
        ...
        with patch(
            ...
            await lexical_retriever.aretrieve(
                RetrievalRequest(
                    "test query", mode="hybrid", rerank=True, use_keyword_extraction=True
                )
            )
...
class TestRedundantResultsAreDropped:
    ...
    def test_lexical_drops_the_duplicate(self):
        ...
        response = retriever.retrieve(
            RetrievalRequest(
                "Hwaseong extreme ultraviolet retrofit",
                mode="lexical",
                top_k=3,
                use_keyword_extraction=False,
            )
    ...
    def test_the_longer_chunk_survives(self):
        ...
        response = retriever.retrieve(
            RetrievalRequest(
                "utilization recovered September",
                mode="lexical",
                top_k=3,
                use_keyword_extraction=False,
            )
    ...
    async def test_the_reranker_gets_at_most_initial_k_candidates(self):
        """Test rerank_initial_k bounds what is sent, as it is documented to."""
        corpus = _overlapping_corpus()
...
class TestDropRedundant:
    ...
    def test_identical_text_under_a_second_id_is_dropped(self):
        ...
        from konte.retrieval.fusion import drop_redundant

        kept = drop_redundant(self._scored("same passage", "same passage"), 10)
    ...
    def test_a_contained_chunk_is_dropped(self):
        ...
        kept = drop_redundant(self._scored("a passage and its tail", "a passage"), 10)
    ...
    def test_a_containing_chunk_survives_a_shorter_one_above_it(self):
        ...
        kept = drop_redundant(self._scored("a passage", "a passage and its tail"), 10)
    ...
    def test_merely_overlapping_chunks_both_survive(self):
        ...
        kept = drop_redundant(self._scored("alpha beta gamma", "beta gamma delta"), 10)
    ...
    def test_ranking_order_is_preserved(self):
        ...
        kept = drop_redundant(self._scored("first", "first", "second", "third"), 10)
